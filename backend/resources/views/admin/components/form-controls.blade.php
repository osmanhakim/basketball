...
<form id="delete-form-{{ $value->id }}" style="display: inline-table;"
    ...
    <button type="button" class="btn btn-sm btn-clean btn-icon m-1" title="{{__('words.delete')}}" data-toggle="modal"
        data-target="#deleteModalSizeSm-{{ $value->id }}">
        <span class="svg-icon svg-icon-md svg-icon-primary">
            <!--begin::Svg Icon | path:assets/media/svg/icons/General/Trash.svg-->
            <i class="fa fa-trash"></i>
        </span>
    </button>
    <div class="modal fade" id="deleteModalSizeSm-{{$value->id}}" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body"> {{ __('message.delete_message') }} <b class="">
                        @if($value->name)
                        {{$value->name}}
