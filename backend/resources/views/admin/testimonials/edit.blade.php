...
@section('title', settings()->website_title . ' | ' . __('words.edit_testimonial'))
@section('breadcrumb')
<div class="d-flex align-items-baseline flex-wrap mr-5">
    ...
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item">
            <span class="text-muted">{{ __('words.edit_testimonial') }}</span>
        </li>
    </ul>
    <!--end::Breadcrumb-->
</div>
...
<div class="card card-custom mb-2">
    <div class="card-header card-header-tabs-line">
        <div class="card-title">
            <h3 class="card-label">{{ __('words.edit_testimonial') }}</h3>
        </div>
            ...
            <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}" role="tabpanel">
                <div class="col form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="{{ $locale . '[name]' }}" placeholder="{{ __('words.name') }}"
                            class="form-control  pl-5 min-h-40px @error($locale . '.name') is-invalid @enderror"
                            value="{{ old($locale . '.name', $testimonial->translate($locale)->name) }}">
                        @error($locale . '[name]')
                ...
                <!-- <div class="col form-group">
                    <label>{{ __('words.description') }} - {{ __('words.locale-' . $locale) }}<span class="text-danger">
                            *
                        </span></label>
                    <div class="input-group">
                        <div class="input-group-prepend">
                        ...
                        <input type="text" name="{{ $locale . '[description]' }}"
                            ...
                            value="{{ old($locale . '.description', $testimonial->translate($locale)->description) }}">
                ...
                <div class="col form-group">
                    ...
                    <textarea class="form-control ckeditor @error($locale . '.description') is-invalid @enderror "
                        ...
                        rows="4">{{ old($locale . '.description', $testimonial->translate($locale)->description) }} </textarea>
                    @error($locale . '[description]')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
